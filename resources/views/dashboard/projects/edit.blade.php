...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('projects.update',$project) }}" method="POST" id="create-project-form">
                @method('PUT')
                @csrf
                <div class="row mb-4">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="partner_id" id="partner_id" class="form-control @error('partner_id') is-invalid @enderror">
                            @foreach ($partners as $partner)
                                <option value="{{ $partner->id }}" @if($partner->id == $project->partner_id) selected @endif>{{ $partner->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-4">
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" name="start_date" id="start_date" class="form-control @error('start_date') is-invalid @enderror" value="{{ date_format($project->start_date, "Y-m-d") }}"/>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" name="end_date" id="end_date" class="form-control @error('end_date') is-invalid @enderror" value="{{ date_format($project->end_date, "Y-m-d") }}"/>
                ...
                <div class="row mb-4">
                    <div class="form-group col-md-4">
                        ...
                        <select name="state_id" id="state_id" class="form-control @error('state_id') is-invalid @enderror" onchange="updateCounties(this, {{ json_encode($states) }})">
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" @if($state->id == $project->state_id) selected @endif>{{ $state->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="county_id" id="county_id" class="form-control @error('county_id') is-invalid @enderror" onchange="updatePayams(this, {{ json_encode($counties) }})">
                            @foreach ($project->state->counties as $county)
                                <option value="{{ $county->id }}" @if($county->id == $project->county_id) selected @endif>{{ $county->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="payam_id" id="payam_id" class="form-control @error('payam_id') is-invalid @enderror">
                            @foreach ($project->county->payams as $payam)
                                <option value="{{ $payam->id }}" @if($payam->id == $project->payam_id) selected @endif>{{ $payam->name }}</option>
                            @endforeach
                        </select>
